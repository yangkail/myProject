...
<!-- 			<th>訂單QR條碼</th> -->
		...
		<c:forEach var="order_rest_tableVO" items="${list}" begin="<%=pageIndex%>"
			...
			<c:if test="${order_rest_tableVO.order_status == 0}">
				<tr>
					...
					<td><c:choose>
							...
							<c:otherwise>
								<button type="button" class="btn_modal"
									id="${order_rest_tableVO.order_id}" disabled="disabled"
									style="background-color: red">訂單已取消</button>
