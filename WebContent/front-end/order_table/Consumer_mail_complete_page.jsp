...
<%
	...
	Order_TableService order_tableSvc = new Order_TableService();
	   List<Order_Rest_TableVO> list = order_tableSvc.getAllConsumer_Complete(gs_email);
	  	if (request.getParameter("gs_email") != null) {
	 		System.out.println("1");
	 		gs_email = request.getParameter("gs_email");
	  	System.out.println("gs_email" + gs_email);
	 		list = order_tableSvc.getAllConsumer_Complete(request.getParameter("gs_email"));
	 	} else {
			System.out.println("2");
 			System.out.println("gs_email"+(String)session.getAttribute("gs_email"));
	  		gs_email = (String) session.getAttribute("gs_email");
	 		list = order_tableSvc.getAllConsumer_Complete((String) session.getAttribute("gs_email"));
	  	}
	session.setAttribute("gs_email", gs_email);
...
<!-- 			<th>訂單QR條碼</th> -->
		...
		<c:forEach var="order_rest_tableVO" items="${list}" begin="<%=pageIndex%>"
			end="<%=pageIndex+rowsPerPage-1%>">
			<c:if test="${order_rest_tableVO.order_status == 1}">
				<tr>
					<td>${order_rest_tableVO.gs_email}</td>
					<td>${order_rest_tableVO.rs_name}</td>
					<td>${order_rest_tableVO.order_id}</td>
					<td>${order_rest_tableVO.order_status==0?"取消":""}
						${order_rest_tableVO.order_status==1?"已接單":""}
						${order_rest_tableVO.order_status==2?"已報到":""}</td>
					<td><fmt:formatDate value="${order_rest_tableVO.order_time}"
							pattern="yyyy-MM-dd HH:mm" /></td>
					<td><fmt:formatDate value="${order_rest_tableVO.order_cancel_time}"
							pattern="yyyy-MM-dd HH:mm" /></td>
...
<!-- 					<td><img -->
<%-- 						src="<%=request.getContextPath()%>/back-end/order_table/order_table.do?action1=display_pic&order_id=${order_rest_tableVO.order_id}" --%>
<!-- 						height="90" width="90"></td> -->
					<%-- 					<td>${order_tableVO.rs_table_status}</td> --%>
					<td><c:choose>
							<c:when test="${order_rest_tableVO.order_status==2}">
								<a href="#">
									<button type="button" class="button_THREE_TYPE" id="ff">填寫意見調查表</button>
								</a>
							</c:when>
							<c:otherwise>
								<button type="button" class="btn_modal"
									id="${order_rest_tableVO.order_id}">取消訂單</button>
