		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/booking_ing/select_pageBK_ing.jsp");
			...
			String url = "/front-end/booking_ing/listOneBK_ing.jsp";
			...
			RequestDispatcher failureView = req.getRequestDispatcher("/front-end/booking_ing/select_pageBK_ing.jsp");
			failureView.forward(req, res);
		}
	}
...
//			String rs_status = req.getParameter("rs_status");
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/front-end/booking_ing/addBK_ing.jsp");
				failureView.forward(req, res);
				return;
			}
			...
			/*************************** 2.開始新增資料 ***************************************/
			Booking_Ing_TableService booking_Ing_TableSvc = new Booking_Ing_TableService();
			booking_Ing_TableVO = booking_Ing_TableSvc.addBooking_Ing_Table(rs_status, rs_sieral, gs_select_time);
			System.out.println("insert");
			/*************************** 3.新增完成,準備轉交(Send the Success view) ***********/
			String url = "/front-end/booking_ing/listAllBK_ing.jsp";
	...
	public void getOne_For_Update(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
			req.setAttribute("booking_Ing_TableVO", booking_Ing_TableVO); // 資料庫取出的empVO物件,存入req
			String url = "/front-end/booking_ing/update_BK_ing_input.jsp";
			...
			RequestDispatcher failureView = req.getRequestDispatcher("/front-end/booking_ing/listAllBK_ing.jsp");
			failureView.forward(req, res);
		}
	}
	...
	public void update(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				...
				req.setAttribute("booking_Ing_TableVO", booking_Ing_TableVO); // 含有輸入格式錯誤的empVO物件,也存入req
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/booking_ing/update_BK_ing_input.jsp");
				failureView.forward(req, res);
				return;
			}
			...
			/*************************** 2.開始修改資料 *****************************************/
			Booking_Ing_TableService booking_Ing_TableSvc = new Booking_Ing_TableService();
			booking_Ing_TableVO = booking_Ing_TableSvc.updateBooking_Ing_Table(rs_sieral, rs_status, order_id,gs_select_time);
			...
			RequestDispatcher failureView = req.getRequestDispatcher("/front-end/booking_ing/update_BK_ing_input.jsp");
			failureView.forward(req, res);
		}
	...
	public void update_status(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			/*************************** 2.開始更改資料 ***************************************/
			Booking_Ing_TableService Booking_Ing_TableSvc = new Booking_Ing_TableService();
			Booking_Ing_TableSvc.updateBooking_Ing_Status(rs_status, order_id,gs_select_time);
			...
			RequestDispatcher failureView = req
					.getRequestDispatcher("/front-end/booking_ing/listAll_Status_BK_ing.jsp");
			failureView.forward(req, res);
		}

	}

	public void get_RS_ID_All(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		// 來自select_page.jsp的請求
		List<String> errorMsgs = new LinkedList<String>();
		// Store this set in the request scope, in case we need to
		// send the ErrorPage view.
		...
		try {
			...
			List<Rest_Seat_Coordinate_TableVO> rest_Seat_Coordinate_TableVO = rest_Seat_Coordinate_TableSvc
					.get_RS_ID_All(rs_id);
