		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/rest_seat_coordinate/select_pageRS_seat.jsp");
			...
			String url = "/front-end/rest_seat_coordinate/listOneRS_seat.jsp";
	...
	public void insert(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/rest_seat_coordinate/addRS_seat.jsp");
				failureView.forward(req, res);
				return;
			}
...
//			System.out.println("insert");
			...
			String url = "/front-end/rest_seat_coordinate/listAllRS_seat.jsp";
			...
			RequestDispatcher failureView = req.getRequestDispatcher("/front-end/rest_seat_coordinate/addRS_seat.jsp");
			failureView.forward(req, res);
		}
	}
	...
	public void getOne_For_Update(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
			req.setAttribute("rest_Seat_Coordinate_TableVO", rest_Seat_Coordinate_TableVO); // 資料庫取出的empVO物件,存入req
			String url = "/front-end/rest_seat_coordinate/update_RS_seat_input.jsp";
			...
			RequestDispatcher failureView = req.getRequestDispatcher("/front-end/rest_seat_coordinate/listAllRS_seat.jsp");
			failureView.forward(req, res);
		}
	}
	...
	public void update(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				...
				req.setAttribute("rest_Seat_Coordinate_TableVO", rest_Seat_Coordinate_TableVO); // 含有輸入格式錯誤的empVO物件,也存入req
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/rest_seat_coordinate/update_RS_seat_input.jsp");
				failureView.forward(req, res);
				return;
			}
			/*************************** 2.開始修改資料 *****************************************/
			...
			RequestDispatcher failureView = req.getRequestDispatcher("/front-end/rest_seat_coordinate/update_RS_seat_input.jsp");
			failureView.forward(req, res);
		}
